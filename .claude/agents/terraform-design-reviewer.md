...
- **指摘文言テンプレ**: 「`<file>:<line>` の属性 `<NAME>` が `ovr.<NAME>` 直接代入になっており、`null` 上書きを許します。`branch_protection.tf:39-63` の `branch_protection` パターン（effective map）に倣い `ovr.<NAME> != null ? ovr.<NAME> : local.branch_protection_preset.<NAME>` 形式に修正してください。」
- **重要度**: blocker

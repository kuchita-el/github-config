...
ベースブランチ名を使い、`.tf` ファイルに限定して差分を取得する:

```bash
git diff <ベースブランチ>...HEAD -- '*.tf'
```

`.tf` 差分がない場合は「Terraform 差分なし」と報告して終了する（観点 8 は plan 出力が提供されていれば評価する）。
...
- **検出条件**: 差分でリソースアドレス（`resource "TYPE" "NAME"` の NAME）または `for_each` キーが変更されており、同一 PR 内に対応する `moved { from = ... to = ... }` ブロックが追加されていない。
- **指摘文言テンプレ**: 「リソース `<TYPE>.<NAME>` の `<アドレス変更|for_each キー変更>` に対し `moved` ブロックがありません。destroy/recreate を防ぐため `moved { from = "<旧アドレス>"; to = "<新アドレス>" }` を追加してください。」
- **重要度**: blocker
- **入出力例**:
  - 陽性: `branch_protection.tf:4` の `resource "github_repository_ruleset" "branch_protection"` を `branch_protection_v2` にリネーム（`moved` なし） → 観点 1 blocker 発火。
  - 陰性: 同リネームに加えて `moved { from = "github_repository_ruleset.branch_protection"; to = "github_repository_ruleset.branch_protection_v2" }` を追加 → 発火しない。
...
- **重要度**: warning
- **指摘文言テンプレ**: 「`resource "<TYPE>" "<NAME>"` で `count = N` が使われていますが、要素が固有のキー（リポジトリ名・ID 等）を持ちます。`count` ではリストの中間要素を削除するとインデックスが再採番され、後続要素が destroy/recreate されます。`for_each = { key => value }` 形式へ変更してください（`branch_protection.tf:4-5` の `for_each = local.branch_protection` パターン参照）。」
...
- **resource 型 × 必要 App 権限の静的テーブル**（観点 7 検出に必要な範囲に絞る、網羅しない）:
  ...
  **テーブル導出元**:
  - `integrations/github` provider 公式ドキュメント（各 resource ページ末尾の "GitHub API Token Scopes" 節）
